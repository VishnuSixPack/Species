...
if (p.species_id) {
  ...
  // Restore the actual per-product Type of Catch / Harvest Method selections
  // AFTER species select (which rebuilds those dropdowns' option lists from
  // the species' comma-separated gear_type/source_type). Previously these
  // two fields were never saved at all — only harvest_method_custom was —
  // so the dropdown selections were silently lost on every edit, and the
  // detail page had nothing to show but the raw species data.
  setTimeout(() => {
    const typeEl = document.getElementById('typeOfCatch');
    if (typeEl && p.type_of_catch) typeEl.value = p.type_of_catch;

    const harvestEl = document.getElementById('harvestMethod');
    if (harvestEl && p.harvest_method) harvestEl.value = p.harvest_method;
...
function collectFormData() {
  ...
  return {
    product: {
      ...
      type_of_catch: document.getElementById('typeOfCatch')?.value || '',
      harvest_method: document.getElementById('harvestMethod')?.value || '',
